...
def compute_bayesian_suggestion(at_index, bayesian_placeholder_phrase, paths, data_types):
    ...
    bayesian_options_bounds_low_frame = pd.read_pickle(
        paths['path_to_bayesian_options_bounds_low_frame'] + '.pkl')
    bayesian_options_bounds_high_frame = pd.read_pickle(
        paths['path_to_bayesian_options_bounds_high_frame'] + '.pkl')
    bayesian_options_round_digits_frame = pd.read_pickle(
        paths['path_to_bayesian_options_round_digits_frame'] + '.pkl')
    ...
    for col_i in range(len(current_log_frame.columns)):
        if current_log_frame.iloc[at_index, col_i] == bayesian_placeholder_phrase:
            ...
            bounds_i = [bayesian_options_bounds_low_frame.loc[at_index, col_name],
                        bayesian_options_bounds_high_frame.loc[at_index, col_name]]
    ...
    for i, col in enumerate(col_name_list):
        if not ma.isnan(bayesian_options_round_digits_frame.loc[at_index, col_name_list[i]]):
            next_sample[i] = round(next_sample[i], int(
                bayesian_options_round_digits_frame.loc[at_index, col_name_list[i]]))
        bayesian_replacement_dict[col_name_list[i]] = next_sample[i].astype(
            data_types[col_name_list[i]]).item()
